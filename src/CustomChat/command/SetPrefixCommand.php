...
class SetPrefixCommand extends Command implements PluginIdentifiableCommand {
    ...
    public function __construct(Main $plugin) {
        ...
        $this->setPermission("customchat.setprefix");
    }
        ...
        if (isset($args[0])) {
            if ($this->plugin->getServer()->getPlayer($args[0])) {
                if (isset($args[1])) {
                    ...
                    $config->set($player->getName() . ".prefix", $args[1]);
                    $config->save();
                    $this->plugin->formatPlayerDisplayName($player);
                    $sender->sendMessage(TextFormat::GREEN . "Set " . $player->getName() . "'s prefix to " . $args[1]);
                } else {
